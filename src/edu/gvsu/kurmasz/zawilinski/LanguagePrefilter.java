...
public class LanguagePrefilter extends TextPrefilter {
    ...
    protected void handleEndTextElement(String uri, String localName,
        ...
        // If we are in the "PRE" stage, pass one more character, just to be sure that
        // the headerSearch isn't sitting at the very end of a header, waiting for one more
        // character to make sure that the header isn't unbalanced.
        //
        // This check is necessary so that the filter doesn't remove language sections with no content
        // (i.e., "<text>==Polish==</text>")
        if (sectionStage == SectionStage.PRE) {
            HeaderSearch.Result content = hs.getCurrentContent();
            if (content != null && headerPattern.matcher(content.headerContent).matches()) {
                String fullHeader = content.fullHeader();
